      ...
      <nav style={{
        ...
        <div className="desktop-nav" style={{ flex: 1, display: 'flex', justifyContent: 'center', gap: '2px' }}>
          {token ? (
            ...
            <>
              <NavLink to="/" style={linkStyle}><Home size={15} />Home</NavLink>
              <a href="#features" style={{ ...linkStyle({ isActive: false }), cursor: 'pointer' }}>Features</a>
              <a href="#about" style={{ ...linkStyle({ isActive: false }), cursor: 'pointer' }}>About</a>
            </>
          )}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
          {!token ? (
            <button onClick={() => navigate('/login')} className="btn btn-primary btn-md">Login / Register</button>
          ) : (
            <>
